...
import com.decooda.dropbox.GetBytes
...
Console.readLine
  ...
  def uploadFileInChunks(): IndexedSeq[Chunk] = {
...
val file = new File("/home/mayank/demoxx1.csv")
  ...
  def uploadFileInChunksCommit = {
    val chunkUploadURL = "https://api-content.dropbox.com/1/commit_chunked_upload/sandbox/yyyy"
    val consumer = Consumer("38cg8jkpl6ycxyt", "zi4gtmnd44sqklr")
    val accessToken = Token("ojcr0eamxsqrth3", "zqxzer9b2x5itou")

    val signedURL = getSignedURL("POST", chunkUploadURL, Map("upload_id" -> "N41xihDfvJG-gfQCZcO3ZA"), consumer, Option(accessToken))
    signedURL
     val result = for (str <- Http(url(signedURL).POST <<?Map("upload_id" -> "N41xihDfvJG-gfQCZcO3ZA")  OK as.String)) yield str
      result.apply
  }
